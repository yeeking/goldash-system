...
class RecordingReceiver 
{
    public:
        ...
        virtual void recordingComplete(File audioFile) = 0;
};
...
class RecordingSender 
{
    public:
        RecordingSender() {}
        virtual void addRecordingReceiver(RecordingReceiver* recordReceiver) = 0;
};
...
class VuMeter : public AudioIODeviceCallback, 
                public Component, 
                public Timer
{
    public: 
                                ...
                                int numberOfSamples) override
        {
            ...
            for (int i = 0; i < numberOfSamples; i+=step)
            
            {
                float inputSample = 0;
                inputSample = inputChannelData[0][i];
                total+=  abs(inputSample); 
            }
            meanDB = total / numberOfSamples * step;
            meanDB = Decibels::gainToDecibels(meanDB);
        }
        ...
        {
            int barWidth = getWidth() - (abs(meanDB / 60.0f) * getWidth());
            if (barWidth <= 0) barWidth = 1;
            
            //std::cout << barWidth << std::endl;
    ...
    void startRecording (const File& file)
    {
        ...
        if (sampleRate > 0)
        {
            ...
            if (auto fileStream = std::unique_ptr<FileOutputStream> (file.createOutputStream()))
            {
                ...
                if (auto writer = oggFormat.createWriterFor (
                    fileStream.get(), 
                    sampleRate, 
                    1, 
                    16, 
                    {}, 
                    8)// output quality for the ogg, 9 is 192
                    )
                {
                    fileStream.release(); // (passes responsibility for deleting the stream to the writer object that is now using it)
...
//        addAndMakeVisible (liveAudioScroller);
        ...
        audioDeviceManager.addAudioCallback (&vuMeter);
    ...
    ~AudioRecordingDemo() override
    {
       ...
       // audioDeviceManager.removeAudioCallback (&liveAudioScroller);
        audioDeviceManager.removeAudioCallback (&vuMeter);

    }

     void addRecordingReceiver(RecordingReceiver* recordReceiver)
    {
    ...
    void resized() override
    {
        ...
        float col = getWidth() / 8;
        //liveAudioScroller.setBounds(0, 0, col, getHeight());
        vuMeter.setBounds(0, 0, col, getHeight());
        
        recordingThumbnail.setBounds (col, 0, col * 4, getHeight());
        recordButton      .setBounds (col * 5, 0, col*3, getHeight());
...
private:
/** notify this object when the recording is complete */
    RecordingReceiver* recordReceiver;
...
//        lastRecording = parentDir.getNonexistentChildFile ("JUCE Demo Audio Recording", ".wav");
        lastRecording = parentDir.getNonexistentChildFile ("dinverno_system_recording", ".ogg");
        // tell the receiver 
        recordReceiver->recordingStarted();
    ...
    {
        recorder.stop();
        recordReceiver->recordingComplete(lastRecording);
...
/*
*/
class RecordWidget    : public Component, 
...
public:
    RecordWidget()
    {
        ...
        recorder.addRecordingReceiver(this);
    }
    ...
    void resized() override
    {
        recorder.setBounds(0, 0, getWidth(), getHeight());
    }

    void recordingStarted() override
    {
        // in case we care!
        std::cout << "RecordWidget::recordingStarted" << std::endl;
        recordReceiver->recordingStarted();
    }

    void recordingComplete(File audioFile) override
    {
        std::cout << "RecordWidget::recordingComplete" << std::endl;
        recordReceiver->recordingComplete(audioFile);
    }
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (RecordWidget)
};
